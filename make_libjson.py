    ...
    for num, line in enumerate(submods, 1):
        ...
        if 'name' in dep and 'version' in dep:
            ...
            if dep['name'] == 'DallasTemperature':
                dep['note'] = 'DallasTemperature - Arduino Library for Dallas Temperature ICs (DS18B20, DS18S20, DS1822, DS1820)'
                dep['library id'] = '54'
                dep['url'] = dep.pop('version')
                dep['authors'] = ["Guil Barros", "Miles Burton", "Rob Tillart", "Tim Nuewsome"]
            if dep['name'] == 'EnableInterrupt':
                dep['note'] = 'GreyGnome\'s EnableInterrupt - Assign an interrupt to any supported pin on all Arduinos'
                dep['library id'] = '311'
                dep['url'] = dep.pop('version')
                dep['authors'] = ["Mike 'GreyGnome' Schwager"]
            if dep['name'] == 'EnviroDIY_DS3231':
                dep['note'] = 'An Arduino library for the DS3231 RTC (Real Time Clock), based off of the Sodaq_DS3231 library.'
                dep['library id'] = '2079'
                dep['url'] = dep.pop('version')
            ...
            if dep['name'] == 'EnviroDIY_ModularSensors':
                dep['note'] = 'EnviroDIY ModularSensors - A library that allows acces to multiple sensors through a unified interface.'
                dep['library id'] = '1648'
                dep['url'] = dep.pop('version')
            ...
            if dep['name'] == 'OneWire':
                dep['note'] = 'OneWire - Control 1-Wire protocol (DS18S20, DS18B20, DS2408 and etc)'
                dep['library id'] = '1'
                dep['url'] = dep.pop('version')
            ...
            if dep['name'] == 'RTCZero':
                dep['note'] = 'Functions for using the processor real time clock in SAMD21 processors'
                dep['library id'] = '873'
                dep['url'] = dep.pop('version')
                dep['authors'] = ["Arduino"]
            if dep['name'] == 'SdFat':
            ...
            if dep['name'] == 'SensorModbusMaster':
                dep['note'] = 'EnviroDIY SensorModbusMaster - Arduino library for communicating via modbus with the Arduino acting as master.'
                dep['library id'] = '1824'
                dep['url'] = dep.pop('version')
            ...
            if dep['name'] == 'YosemitechModbus':
                dep['note'] = 'Arduino library for communication with Yosemitech sensors via Modbus.'
                dep['library id'] = '2078'
                dep['url'] = dep.pop('version')
                dep['authors'] = ["Sara Damiano"]
