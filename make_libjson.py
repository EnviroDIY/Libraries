    ...
    for num, line in enumerate(submods, 1):
        ...
        if 'name' in dep and 'version' in dep:
            ...
            if dep['name'] == 'Adafruit_AM2315':
                ...
                dep['note'] = 'AOSong AM2315 I2C Temp/Humidity Sensor \
                Library by Adafruit'
                dep['library id'] = '773'
                dep['url'] = dep.pop('version')
                dep['authors'] = ["Adafruit"]
            if dep['name'] == 'Adafruit_BME280':
                ...
                dep['note'] = 'Bosch BME280 Temp/Humidity/Pressure Sensor \
                Library by Adafruit'
                dep['library id'] = '166'
                dep['url'] = dep.pop('version')
                dep['authors'] = ["Adafruit"]
            ...
            if dep['name'] == 'DallasTemperature':
                ...
                Dallas Temperature ICs (DS18B20, DS18S20, DS1822, DS1820)'
                dep['library id'] = '54'
                dep['url'] = dep.pop('version')
            ...
            if dep['name'] == 'EnableInterrupt':
                ...
                Assign an interrupt to any supported pin on all Arduinos'
                dep['library id'] = '311'
                dep['url'] = dep.pop('version')
                dep['authors'] = ["Mike 'GreyGnome' Schwager"]
            if dep['name'] == 'EnviroDIY_DS3231':
                dep['note'] = 'An Arduino library for the DS3231 RTC \
                (Real Time Clock), based off of the Sodaq_DS3231 library.'
                dep['library id'] = '2079'
                dep['url'] = dep.pop('version')
            ...
            if dep['name'] == 'EnviroDIY_ModularSensors':
                ...
                allows access to multiple sensors through a unified interface.'
                dep['library id'] = '1648'
                dep['url'] = dep.pop('version')
                dep['authors'] = ["Sara Damiano", "Shannon Hicks"]
            if dep['name'] == 'EnviroDIY_PCInt_PCINT0':
                dep['name'] = 'Sodaq_PcInt_PCINT0'
                dep['note'] = 'Sodaq library controlling pin change interrupts\
                 - modified to use only interrupt vector 0.'
                dep['authors'] = ["Kees Bakker"]
            ...
            if dep['name'] == 'EnviroDIY_SoftwareSerial_ExtInts':
                ...
                dep['note'] = 'Arduino SoftwareSerial  Library - modified to \
                use interrupts called by an external library.'
            if dep['name'] == 'EnviroDIY_SoftwareSerial_PCINT12':
                dep['name'] = 'SoftwareSerial_PCINT12'
                dep['note'] = 'Arduino SoftwareSerial  Library - modified to \
                use only interrupt vectors 1 and 2.'
            if dep['name'] == 'OneWire':
            ...
            if dep['name'] == 'SensorModbusMaster':
                dep['note'] = 'EnviroDIY SensorModbusMaster - Arduino library \
                for communicating via modbus with the Arduino acting as the \
                modbus master.'
                dep['library id'] = '1824'
                dep['url'] = dep.pop('version')
